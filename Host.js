...
// const MongoDBStore = require("connect-mongo-db-session")(session); //import the mongoDB session store

const { MongoClient, ServerApiVersion } = require('mongodb');
const uri = "mongodb+srv://helloworld:olanam123@cluster0.xttwcta.mongodb.net/?retryWrites=true&w=majority";
...
async function run() {
  try {
    ...
    // Use the MongoDB client for database operations
    const database = client.db("swc4u"); // Replace with your MongoDB database name
    const collection = database.collection("Student Accounts"); // Replace with your MongoDB collection name
...
myapp.post('/register', async (req, res) => {
  ...
  try {
    ...
    const AccountsCollection = client.db("swc4u").collection(accountType === 'Student' && 'Counselor' ? 'Student Accounts' : 'Counselor Accounts');
    const emailExists = await AccountsCollection.findOne({ email: uppercaseEmail });

    if (emailExists) {
      res.status(400).json({
        error: 'The email is already registered. Please log in or reset your password if you forgot it.',
      });
      return;
    }

    // Insert user data into MongoDB
    const userData = await AccountsCollection.insertOne({
      first_name: uppercaseFirstName,
      last_name: uppercaseLastName,
      birth_date: birthDate,
      gender: uppercaseGender,
...
myapp.post('/studentCancelAppointment/:appointmentId', async (req, res) => {
  try {
    ...
    // Retrieve appointment details from MongoDB based on appointmentId
    const appointmentDetails = await appointmentCollection.findOne({ _id: appointmentId });

    if (!appointmentDetails) {
      return res.status(404).send('Appointment not found');
    }

    const { email, first_name, last_name, department, appointed_date, appointed_time } = appointmentDetails;

    // Prepare data for 'Appointment History'
    ...
    // Save cancelled appointment in 'Appointment History'
    await appointmentCollection.insertOne(cancelledAppointmentData);

    // Remove the appointment from 'Pending Appointment'
